/******/ (function(modules) { // webpackBootstrap
/******/ 	// The module cache
/******/ 	var installedModules = {};
/******/
/******/ 	// The require function
/******/ 	function __webpack_require__(moduleId) {
/******/
/******/ 		// Check if module is in cache
/******/ 		if(installedModules[moduleId]) {
/******/ 			return installedModules[moduleId].exports;
/******/ 		}
/******/ 		// Create a new module (and put it into the cache)
/******/ 		var module = installedModules[moduleId] = {
/******/ 			i: moduleId,
/******/ 			l: false,
/******/ 			exports: {}
/******/ 		};
/******/
/******/ 		// Execute the module function
/******/ 		modules[moduleId].call(module.exports, module, module.exports, __webpack_require__);
/******/
/******/ 		// Flag the module as loaded
/******/ 		module.l = true;
/******/
/******/ 		// Return the exports of the module
/******/ 		return module.exports;
/******/ 	}
/******/
/******/
/******/ 	// expose the modules object (__webpack_modules__)
/******/ 	__webpack_require__.m = modules;
/******/
/******/ 	// expose the module cache
/******/ 	__webpack_require__.c = installedModules;
/******/
/******/ 	// define getter function for harmony exports
/******/ 	__webpack_require__.d = function(exports, name, getter) {
/******/ 		if(!__webpack_require__.o(exports, name)) {
/******/ 			Object.defineProperty(exports, name, { enumerable: true, get: getter });
/******/ 		}
/******/ 	};
/******/
/******/ 	// define __esModule on exports
/******/ 	__webpack_require__.r = function(exports) {
/******/ 		if(typeof Symbol !== 'undefined' && Symbol.toStringTag) {
/******/ 			Object.defineProperty(exports, Symbol.toStringTag, { value: 'Module' });
/******/ 		}
/******/ 		Object.defineProperty(exports, '__esModule', { value: true });
/******/ 	};
/******/
/******/ 	// create a fake namespace object
/******/ 	// mode & 1: value is a module id, require it
/******/ 	// mode & 2: merge all properties of value into the ns
/******/ 	// mode & 4: return value when already ns object
/******/ 	// mode & 8|1: behave like require
/******/ 	__webpack_require__.t = function(value, mode) {
/******/ 		if(mode & 1) value = __webpack_require__(value);
/******/ 		if(mode & 8) return value;
/******/ 		if((mode & 4) && typeof value === 'object' && value && value.__esModule) return value;
/******/ 		var ns = Object.create(null);
/******/ 		__webpack_require__.r(ns);
/******/ 		Object.defineProperty(ns, 'default', { enumerable: true, value: value });
/******/ 		if(mode & 2 && typeof value != 'string') for(var key in value) __webpack_require__.d(ns, key, function(key) { return value[key]; }.bind(null, key));
/******/ 		return ns;
/******/ 	};
/******/
/******/ 	// getDefaultExport function for compatibility with non-harmony modules
/******/ 	__webpack_require__.n = function(module) {
/******/ 		var getter = module && module.__esModule ?
/******/ 			function getDefault() { return module['default']; } :
/******/ 			function getModuleExports() { return module; };
/******/ 		__webpack_require__.d(getter, 'a', getter);
/******/ 		return getter;
/******/ 	};
/******/
/******/ 	// Object.prototype.hasOwnProperty.call
/******/ 	__webpack_require__.o = function(object, property) { return Object.prototype.hasOwnProperty.call(object, property); };
/******/
/******/ 	// __webpack_public_path__
/******/ 	__webpack_require__.p = "";
/******/
/******/
/******/ 	// Load entry module and return exports
/******/ 	return __webpack_require__(__webpack_require__.s = "./js/main.js");
/******/ })
/************************************************************************/
/******/ ({

/***/ "./js/main.js":
/*!********************!*\
  !*** ./js/main.js ***!
  \********************/
/*! no exports provided */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _modules_smooth_scroll__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./modules/smooth-scroll */ "./js/modules/smooth-scroll.js");
/* harmony import */ var _modules_phone_mask__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./modules/phone-mask */ "./js/modules/phone-mask.js");
/* harmony import */ var _modules_validate_form__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./modules/validate-form */ "./js/modules/validate-form.js");
/* harmony import */ var _modules_popup__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/popup */ "./js/modules/popup.js");
/* harmony import */ var _modules_success__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./modules/success */ "./js/modules/success.js");
/* harmony import */ var _modules_accordion__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./modules/accordion */ "./js/modules/accordion.js");






Object(_modules_smooth_scroll__WEBPACK_IMPORTED_MODULE_0__["initSmoothScroll"])();
Object(_modules_phone_mask__WEBPACK_IMPORTED_MODULE_1__["initPhoneMask"])();
Object(_modules_validate_form__WEBPACK_IMPORTED_MODULE_2__["initValidateForms"])();
Object(_modules_popup__WEBPACK_IMPORTED_MODULE_3__["initPopup"])();
Object(_modules_success__WEBPACK_IMPORTED_MODULE_4__["initSuccess"])();
Object(_modules_accordion__WEBPACK_IMPORTED_MODULE_5__["initAccordion"])();

/***/ }),

/***/ "./js/modules/accordion.js":
/*!*********************************!*\
  !*** ./js/modules/accordion.js ***!
  \*********************************/
/*! exports provided: initAccordion */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "initAccordion", function() { return initAccordion; });
var accordion = document.querySelector('.accordion');
var accordionContents = document.querySelectorAll('.accordion__content');
var accordionButtons = document.querySelectorAll('.accordion__button');

var initJs = function initJs() {
  accordionContents.forEach(function (accordionContent) {
    accordionContent.classList.add('is-hidden');
  });
  accordionButtons.forEach(function (accordionButton) {
    accordionButton.classList.add('is-js');
  });
};

var initAccordion = function initAccordion() {
  if (!accordion) {
    return;
  }

  initJs();
  var accordionItems = accordion.querySelectorAll('.accordion__item');
  accordion.addEventListener('click', function (evt) {
    if (evt.target.classList.contains('accordion__button')) {
      var parent = evt.target.closest('.accordion__item');

      if (parent.classList.contains('is-active')) {
        parent.classList.remove('is-active');
        return;
      }

      accordionItems.forEach(function (el) {
        return el.classList.remove('is-active');
      });
      parent.classList.add('is-active');
    }
  });
};

/***/ }),

/***/ "./js/modules/phone-mask.js":
/*!**********************************!*\
  !*** ./js/modules/phone-mask.js ***!
  \**********************************/
/*! exports provided: initPhoneMask */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "initPhoneMask", function() { return initPhoneMask; });
var phoneInputs = document.querySelectorAll('input[type="tel"]');
var COUNTRY_CODE = '+7 (';
var length = COUNTRY_CODE.length;

var replacePhoneValue = function replacePhoneValue(el) {
  var matrix = "".concat(COUNTRY_CODE, "___) ___ __ __");
  var def = matrix.replace(/\D/g, '');
  var i = 0;
  var val = el.value.replace(/\D/g, '');

  if (def.length >= val.length) {
    val = def;
  }

  el.value = matrix.replace(/./g, function (a) {
    // eslint-disable-next-line no-nested-ternary
    return /[_\d]/.test(a) && i < val.length ? val.charAt(i++) : i >= val.length ? '' : a;
  });
};

var onInputPhoneInput = function onInputPhoneInput(_ref) {
  var target = _ref.target;
  replacePhoneValue(target);
};

var onFocusPhoneInput = function onFocusPhoneInput(_ref2) {
  var target = _ref2.target;

  if (!target.value) {
    target.value = COUNTRY_CODE;
    target.addEventListener('input', onInputPhoneInput);
    target.addEventListener('blur', onBlurPhoneInput);
    target.addEventListener('keydown', onKeydownPhoneInput);
  }
};

var onKeydownPhoneInput = function onKeydownPhoneInput(e) {
  if (e.target.selectionStart <= length && e.keyCode !== 8 && e.keyCode !== 46) {
    e.target.setSelectionRange(length, length);
  }

  if ((e.target.selectionStart === length || e.target.selectionStart === 1) && e.keyCode === 8) {
    e.preventDefault();
  }

  if (e.target.selectionStart === 1 && e.keyCode === 46) {
    e.preventDefault();
  }
};

var onBlurPhoneInput = function onBlurPhoneInput(_ref3) {
  var target = _ref3.target;

  if (target.value === COUNTRY_CODE) {
    target.value = '';
    target.removeEventListener('input', onInputPhoneInput);
    target.removeEventListener('blur', onBlurPhoneInput);
  }
};

var initPhoneMask = function initPhoneMask() {
  if (phoneInputs.length) {
    phoneInputs.forEach(function (input) {
      input.addEventListener('focus', onFocusPhoneInput);

      if (input.value) {
        replacePhoneValue(input);
        input.addEventListener('input', onInputPhoneInput);
        input.addEventListener('blur', onBlurPhoneInput);
        input.addEventListener('keydown', onKeydownPhoneInput);
      }
    });
  }
};

/***/ }),

/***/ "./js/modules/popup.js":
/*!*****************************!*\
  !*** ./js/modules/popup.js ***!
  \*****************************/
/*! exports provided: hidePopup, initPopup */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "hidePopup", function() { return hidePopup; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "initPopup", function() { return initPopup; });
/* harmony import */ var _utils_utils__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../utils/utils */ "./js/utils/utils.js");

var buttonFeedback = document.querySelector('.header__link-feedback');
var overlayPopup = document.querySelector('.popup-overlay');
var buttonClosePopup = document.querySelector('.popup__button-close');
var userNames = document.querySelectorAll('[name=name]');
var userPhones = document.querySelectorAll('[name=phone]');
var userQuestions = document.querySelectorAll('[name=permission]');
var storage = localStorage.getItem('name');

var onButtonFeedbackClick = function onButtonFeedbackClick() {
  if (!overlayPopup.classList.contains('popup-active')) {
    overlayPopup.classList.add('popup-active');
  }

  document.body.classList.add('scroll-lock');
  userNames.forEach(function (userName) {
    userPhones.forEach(function (userPhone) {
      userQuestions.forEach(function (userQuestion) {
        if (storage) {
          userName.value = storage;
        } else if (storage) {
          userPhone.value = storage;
          userQuestion.focus();
        } else {
          userPhone.focus();
        }

        userName.focus();
      });
    });
  });
};

var hidePopup = function hidePopup() {
  if (overlayPopup.classList.contains('popup-active')) {
    overlayPopup.classList.remove('popup-active');
  }

  document.body.classList.remove('scroll-lock');
};

var onButtonCloseClick = function onButtonCloseClick() {
  return hidePopup();
};

var onKeyEscKeydown = function onKeyEscKeydown(evt) {
  if (Object(_utils_utils__WEBPACK_IMPORTED_MODULE_0__["isEscEvent"])(evt)) {
    hidePopup();
  }
};

var onOverlayClick = function onOverlayClick(evt) {
  if (!evt.target.closest('.popup')) {
    hidePopup();
  }
};

var initPopup = function initPopup() {
  if (!overlayPopup) {
    return;
  }

  buttonFeedback.addEventListener('click', onButtonFeedbackClick);
  buttonClosePopup.addEventListener('click', onButtonCloseClick);
  document.addEventListener('keydown', onKeyEscKeydown);
  overlayPopup.addEventListener('click', onOverlayClick);
};

/***/ }),

/***/ "./js/modules/smooth-scroll.js":
/*!*************************************!*\
  !*** ./js/modules/smooth-scroll.js ***!
  \*************************************/
/*! exports provided: initSmoothScroll */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "initSmoothScroll", function() { return initSmoothScroll; });
var move = new window.MoveTo({
  duration: 2000,
  easing: 'easeOutQuart'
});
var initSmoothScroll = function initSmoothScroll() {
  var smoothLinks = document.querySelectorAll('.main-screen__link');

  if (smoothLinks.length) {
    smoothLinks.forEach(function (link) {
      move.registerTrigger(link);
      document.activeElement.blur();
    });
  }
};

/***/ }),

/***/ "./js/modules/success.js":
/*!*******************************!*\
  !*** ./js/modules/success.js ***!
  \*******************************/
/*! exports provided: openSuccess, hideSuccess, initSuccess */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "openSuccess", function() { return openSuccess; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "hideSuccess", function() { return hideSuccess; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "initSuccess", function() { return initSuccess; });
/* harmony import */ var _utils_utils__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../utils/utils */ "./js/utils/utils.js");

var overlaySuccess = document.querySelector('.success-overlay');
var buttonCloseSuccess = document.querySelector('.success__button-close');
var openSuccess = function openSuccess() {
  if (!overlaySuccess.classList.contains('success-active')) {
    overlaySuccess.classList.add('success-active');
  }

  document.body.classList.add('scroll-lock');
};
var hideSuccess = function hideSuccess() {
  if (overlaySuccess.classList.contains('success-active')) {
    overlaySuccess.classList.remove('success-active');
  }

  document.body.classList.remove('scroll-lock');
};

var onButtonCloseSuccessClick = function onButtonCloseSuccessClick() {
  return hideSuccess();
};

var onKeyEscKeydown = function onKeyEscKeydown(evt) {
  if (Object(_utils_utils__WEBPACK_IMPORTED_MODULE_0__["isEscEvent"])(evt)) {
    hideSuccess();
  }
};

var onOverlayClick = function onOverlayClick(evt) {
  if (!evt.target.closest('.success')) {
    hideSuccess();
  }
};

var initSuccess = function initSuccess() {
  if (!overlaySuccess) {
    return;
  }

  buttonCloseSuccess.addEventListener('click', onButtonCloseSuccessClick);
  document.addEventListener('keydown', onKeyEscKeydown);
  overlaySuccess.addEventListener('click', onOverlayClick);
};

/***/ }),

/***/ "./js/modules/validate-form.js":
/*!*************************************!*\
  !*** ./js/modules/validate-form.js ***!
  \*************************************/
/*! exports provided: initValidateForms */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "initValidateForms", function() { return initValidateForms; });
/* harmony import */ var _popup__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./popup */ "./js/modules/popup.js");
/* harmony import */ var _success__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./success */ "./js/modules/success.js");


var userNames = document.querySelectorAll('[name=name]');
var userPhones = document.querySelectorAll('[name=phone]');
var userQuestions = document.querySelectorAll('textarea');

var removeError = function removeError(form) {
  var errors = form.querySelectorAll('.check-input');
  errors.forEach(function (error) {
    error.classList.remove('check-input');
  });
};

var checkInputsNameValidity = function checkInputsNameValidity(inputs) {
  var flag = true;
  inputs.forEach(function (input) {
    if (!input.value) {
      input.classList.add('check-input');
      flag = false;
      return;
    }

    input.classList.remove('check-input');
  });
  return flag;
};

var checkInputsPhoneValidity = function checkInputsPhoneValidity(inputs) {
  var flag = true;
  inputs.forEach(function (input) {
    if (input.value.length < 18) {
      input.classList.add('check-input');
      flag = false;
      return;
    }

    input.classList.remove('check-input');
  });
  return flag;
};

var checkInputsCheckboxValidity = function checkInputsCheckboxValidity(inputs) {
  var flag = true;
  inputs.forEach(function (input) {
    if (!input.checked) {
      input.classList.add('check-input');
      flag = false;
      return;
    }

    input.classList.remove('check-input');
  });
  return flag;
};

var addLocalStorage = function addLocalStorage() {
  if (userNames) {
    userNames.forEach(function (userName) {
      localStorage.setItem('name', userName.value);
    });
  }

  if (userPhones) {
    userPhones.forEach(function (userPhone) {
      localStorage.setItem('phone', userPhone.value);
    });
  }

  if (userQuestions) {
    userQuestions.forEach(function (userQuestion) {
      localStorage.setItem('text', userQuestion.value);
    });
  }
};

var onFormSubmit = function onFormSubmit(evt) {
  evt.preventDefault();
  var inputsName = evt.target.querySelectorAll('[name=name]');
  var inputsPhone = evt.target.querySelectorAll('[name=phone]');
  var inputsCheckbox = evt.target.querySelectorAll('[name=permission]');

  var checkInputsValidity = function checkInputsValidity() {
    var flag = true;

    if (!checkInputsNameValidity(inputsName) || !checkInputsPhoneValidity(inputsPhone) || !checkInputsCheckboxValidity(inputsCheckbox)) {
      flag = false;
    }

    return flag;
  };

  if (checkInputsValidity()) {
    removeError(evt.target);
    addLocalStorage();
    setTimeout(function () {
      evt.target.reset();
      Object(_popup__WEBPACK_IMPORTED_MODULE_0__["hidePopup"])();
      Object(_success__WEBPACK_IMPORTED_MODULE_1__["openSuccess"])();
    }, 1000);
  }
};

var initValidateForms = function initValidateForms() {
  var forms = document.querySelectorAll('form');
  forms.forEach(function (form) {
    form.noValidate = true;
    form.addEventListener('submit', onFormSubmit);
  });
};

/***/ }),

/***/ "./js/utils/utils.js":
/*!***************************!*\
  !*** ./js/utils/utils.js ***!
  \***************************/
/*! exports provided: isEscEvent */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "isEscEvent", function() { return isEscEvent; });
// Функция, проверяющая нажатую клавишу 'Esc'
var isEscEvent = function isEscEvent(evt) {
  return evt.key === 'Escape' || evt.key === 'Esc';
};

/***/ })

/******/ });
//# sourceMappingURL=main.min.js.map